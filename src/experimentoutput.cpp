...
void ExperimentOutput::process()
{
    ...
    */
    ...
    {
        std::cout << MSEerrorAmplitude << " -> MSE error Amplitude" << std::endl;
...
void ExperimentOutput::displayGlobalResults()
{
    ...
    std::cout 	<< std::endl << "<===== SPRIGHT RESULTS =====>" << std::endl;
...
bool ExperimentOutput::isBinningFailure(std::set<int> missedLocations) const
{
    ...
    */
    ...
    {
        // std::vector<int> binStatus(config->getOldBinSize(stage), 0);
        std::vector<int> binStatus(config->getBinNumItems(stage), 0);


        for (auto it = missedLocations.cbegin(); it != missedLocations.cend(); ++it)
        {
            // binStatus[*it % config->getOldBinSize(stage)]++;
            binStatus[*it % config->getBinNumItems(stage)]++;
        }
